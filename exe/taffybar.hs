...
workspaces =
  ...
  where
    -- namedScratchpadFilterOutWorkspace not working completly
    -- filter out here
    myShowWorkspaceFn w = workspaceName w /= "NSP"
...
segmentColor n = foldl' go Nothing
  where
    go acc (val, col) = if n >= val then Just col else acc
...
markText attrs t = "<span " <> tags <> ">" <> t <> "</span>"
  where
    tags = T.pack $ unwords $ show <$> attrs
